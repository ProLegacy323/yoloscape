...
public class client extends RSApplet {
	...
	public int screenSizeOldX = 765;
	public int screenSizeOldY = 503;
	public int screenSizeX = 765;
	public int screenSizeY = 503;
	...
	public void init() {
		...
		setHighMem();
		isMembers = true;
		initClientFrame(screenSizeY, screenSizeX);
	}
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
 ...
 //	   myPassword = "";
	...
	void drawLoadingText(int i, String s)
	{
		...
		char c1 = '\310';
		byte byte1 = 20;
		chatTextDrawingArea.drawText(0xffffff, "Yoloscape is loading - please cotch...", c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
  ...
  */
	...
	private void dropClient()
	{
		...
		aRSImageProducer_1165.initDrawingArea();
		aTextDrawingArea_1271.drawText(0, "I HATE THIS ERROR SO MUCH", 144, 257);
		aTextDrawingArea_1271.drawText(0xffffff, "I HATE THIS ERROR SO MUCH", 143, 256);
		aTextDrawingArea_1271.drawText(0, "IT TOOK AGES TO DEBUG THIS ARGH", 159, 257);
		aTextDrawingArea_1271.drawText(0xffffff, "IT TOOK AGES TO DEBUG THIS ARGH", 158, 256);
		aTextDrawingArea_1271.drawText(0, "Also, lost connection to server, reestablishing. -Figgle", 174, 257);
		aTextDrawingArea_1271.drawText(0xffffff, "Also, lost connection to server, reestablishing. -Figgle", 173, 256);
		aRSImageProducer_1165.drawGraphics(4, super.graphics, 4);
		anInt1021 = 0;
		destX = 0;
		...
		do {
			...
			if(openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if((j == 13 || j == 10) && inputString.length() > 0) {
					if(myPrivilege == 2 || server.equals(server) || 1 == 1/*to remove*/) {
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9)
	{
		...
		if(j == 653)
			...
			if(daysSinceRecovChange == 200)
				class9.message = "We strongly recommend you do so now to secure your SwagBank(TM).";
			else
		...
		if(j == 654)
		{
			...
			if(daysSinceRecovChange == 200)
			{
				class9.message = "Do this from the 'SwagBank(TM) management' area on our front webpage";
				return;
			}
			class9.message = "Do this from the 'SwagBank(TM) management' area on our front webpage";
		}
	...
	public String getDocumentBaseHost() {
		...
		if (super.gameFrame != null) {
			return ""; // Yoloscape.swag <- removed for Jframe to work
		} else {
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			{
				loginMessage1 = "";
				loginMessage2 = "Invalid swaggotname or password.";
				return;
			}
			if(k == 4)
			{
				loginMessage1 = "Your SwagBank(TM) has been disabled.";
				loginMessage2 = "Please check your message-center for details.";
				return;
			}
			if(k == 5)
			{
				loginMessage1 = "Your SwagBank(TM) is already logged in.";
				loginMessage2 = "Try again in 60 secs...";
				return;
			}
			if(k == 6)
			{
				loginMessage1 = "Yoloscape has been updated!";
				loginMessage2 = "Please reload this page.";
				return;
			}
			if(k == 7)
			{
				loginMessage1 = "Your mother is full.";
				loginMessage2 = "Please empty her and try again.";
				return;
			}
			if(k == 8)
			{
				loginMessage1 = "Unable to connect.";
				loginMessage2 = "lol phagget server is down, wasteman";
				return;
			}
			if(k == 9)
			{
				loginMessage1 = "CotChin limit exceeded.";
				loginMessage2 = "Stop hitting on Yoloscape.";
				return;
			}
			if(k == 10)
			{
			...
			if(k == 11)
			{
				loginMessage2 = "CotChin server rejected you.";
				loginMessage2 = "Don't bother trying again.";
				return;
			}
			if(k == 12)
			{
				loginMessage1 = "You need a proper swaggot SwagBank(TM) to CotChin to this world.";
				loginMessage2 = "Please become a real swaggot, or use a different world.";
				return;
			}
			if(k == 13)
			{
				loginMessage1 = "Could not complete CotChin.";
			...
			if(k == 14)
			{
				...
				loginMessage2 = "Cotch for a minute and try again.";
				return;
			}
			if(k == 15)
			{
			...
			if(k == 16)
			{
				loginMessage1 = "CotChin attempts exceeded.";
				loginMessage2 = "You've tried to enter me more times than I did your mother. Wait a minute.";
				return;
			}
			if(k == 17)
			{
				loginMessage1 = "You are standing in a proper-swaggot-only area.";
				loginMessage2 = "To play on this world move to a peasent cunt swaggot area first";
				return;
			}
			if(k == 20)
			{
				loginMessage1 = "Invalid CotChinserver requested";
				loginMessage2 = "Please try using a different world.";
				return;
			}
			if(k == 21)
		...
		loginMessage2 = "Error connecting to server. lol chins";
	}
...
//	entityMessage(npc);
 ...
 //		   return;
		...
		if(signlink.cache_dat != null)
		{
			...
			Texture.getDistancePlayerModel(765, 503);
			fullScreenTextureArray = Texture.anIntArray1472;
			Texture.getDistancePlayerModel(519, 165);
			anIntArray1180 = Texture.anIntArray1472;
			Texture.getDistancePlayerModel(246, 335);
			anIntArray1181 = Texture.anIntArray1472;
			Texture.getDistancePlayerModel(512, 334);
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading Yoloscape", 30, k);
			k += 50;
		...
		if(genericLoadingError)
		{
			...
			g.drawString("To play Yoloscape make sure you play from", 50, 100);
			g.drawString("http://www.yoloscape.swag", 50, 150);
		}
		if(rsAlreadyLoaded)
		{
			aBoolean831 = false;
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of Yoloscape already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
			...
			if (loadingStage == 2) {
				...
				if (openInterfaceID != -1) {
					...
					if (rsInterface_1.width == 512 && rsInterface_1.height == 334 && rsInterface_1.type == 0) {
						rsInterface_1.width = screenSizeX;
						rsInterface_1.height = screenSizeY;
					}
					drawInterface(0, 0, rsInterface_1, 8);
				}
				RSInterface rsInterface = RSInterface.interfaceCache[fullscreenInterfaceID];
				if (rsInterface.width == 512 && rsInterface.height == 334 && rsInterface.type == 0) {
					rsInterface.width = screenSizeX;
					rsInterface.height = screenSizeY;
				}
 ...
 //			   return new DataInputStream((new URL(getCodeBase(), s)).openStream());
	...
	private void processLoginScreenInput()
	{
		if(loginScreenState == 0)
		{
			...
			{
				loginMessage1 = "";
				loginMessage2 = "Enter your swaggotname & password.";
				loginScreenState = 2;
				loginScreenCursorPos = 0;
			}
		} else
...
*/
	...
	public void resetAllImageProducers() {
		...
		aRSImageProducer_1114 = null;
		aRSImageProducer_1115 = null;
		super.fullGameScreen = new RSImageProducer(1280, 720, getGameComponent());
		welcomeScreenRaised = true;
	}
		...
		try { 
			if (osName.startsWith("Mac OS")) { 
				Class fileMgr = Class.forName("com.apple.eio.FileManager"); 
                                Class<?>[] strings1 = new Class[0];
                                strings1[0] = String.class;
				Method openURL = fileMgr.getDeclaredMethod("openURL", strings1[0]); 
